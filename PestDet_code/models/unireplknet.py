...
__all__ = ['unireplknet_a', 'unireplknet_f', 'unireplknet_p', 'unireplknet_pt','unireplknet_n', 'unireplknet_t', 'unireplknet_s', 'unireplknet_b', 'unireplknet_l', 'unireplknet_xl']
...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, k=1, s=1, p=None, g=1, act=True,reduction=8):
        super(CoordAtt, self).__init__()
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))  # [b,c,h,1]
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))  # [b,c,1,w]

        mip = max(12, inp // reduction)  # 输入channel数除以reduction
    ...
    def forward(self, inputs):
        ...
        def _f(x):
            if self.need_contiguous:
                x = x.contiguous()
            y = self.CA(self.norm(self.dwconv(x)))
...
default_UniRepLKNet_A_F_P_kernel_sizes = ((3, 3),
                                      ...
                                      (13, 13, 13, 13, 13, 13),
                                      (13, 13))
default_UniRepLKNet_PT_kernel_sizes = ((3, 3),
                                      (13, 13),
...
UniRepLKNet_PT_depths = (2, 2, 12, 2)
...
default_depths_to_kernel_sizes = {
    ...
    UniRepLKNet_PT_depths: default_UniRepLKNet_PT_kernel_sizes,
...
def unireplknet_pt(weights='', **kwargs):
    model = UniRepLKNet(depths=UniRepLKNet_PT_depths, dims=(64, 128, 256, 512), **kwargs)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)))
    return model
